...
func run_dialogue():
	if player.finish_type1 and player.finish_type2:
		dialogue_objective.start('res://dialogs/type3_oba.json')
		player.is_have_herb = true
	else:
		dialogue_objective.start('res://dialogs/type3_oba_f.json')
